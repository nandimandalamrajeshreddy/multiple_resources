...
MP_PKG="morpheus-appliance-4.2.5-1.el7.x86_64.rpm"
MP_URL_DN="https://downloads.morpheusdata.com/files/morpheus-appliance-4.2.5-1.el7.x86_64.rpm"
SSHPAS="XXXX*XXX"
##################  CONFIG Section ###########################################################
...
#	echo -e "$(date "+%m%d%Y %T") : ------------ run command $1"
    ...
    run_cmd hostname -i > /tmp/ip3
	run_cmd sed -i '2d' /tmp/ip3
...
source /root/.bashrc
	...
	run_cmd sleep 5
source /root/.bashrc
sshpass -e scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no /tmp/ip3 cmpadmin@cmpnode-1:/tmp/
sshpass -e scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no /tmp/ip3 cmpadmin@cmpnode-2:/tmp/
	echo "####################################################################################"
