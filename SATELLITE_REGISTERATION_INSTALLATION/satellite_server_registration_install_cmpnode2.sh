...
#	echo -e "$(date "+%m%d%Y %T") : ------------ run command $1"
	...
	run_cmd hostname -i > /tmp/ip2
	run_cmd sed -i '2d' /tmp/ip2
	...
	run_cmd subscription-manager register --org="XXXXX" --activationkey="XXXX-XXXX" --force
...
source /root/.bashrc
	...
	run_cmd sleep 5
source /root/.bashrc
sshpass -e scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no /tmp/ip2 cmpadmin@cmpnode-1:/tmp/
sshpass -e scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no /tmp/ip2 cmpadmin@cmpnode-3:/tmp/
	echo "########################################################################################################"
