...
EOF
        ...
        run_cmd sleep 30
mysql  -e "ALTER USER 'root'@'localhost' IDENTIFIED BY '${MYPAS}'" --connect-expired-password
        run_cmd sed -i '$d' /root/.my.cnf
        run_cmd echo "password=${MYPAS}" >> /root/.my.cnf
        run_cmd sed -i '$d' /root/.my.cnf
        run_cmd sleep 30
mysql  -e "CREATE USER 'sstuser'@'localhost' IDENTIFIED BY '${SSTPAS}'"
...
awk 'BEGIN { ORS = "," } { print }' /tmp/raw.dat > /tmp/cmpnode1.dat
echo "awk command ran sucessfully************"
#       run_cmd sed -i '$d' /tmp/cmpnode1.dat
        run_cmd truncate -s-1 /tmp/cmpnode1.dat
export gcomm=`echo gcomm://$(cat /tmp/cmpnode1.dat)`
...
export wsrepname=`echo wsrep_node_name=$(hostname)`
        ...
        run_cmd systemctl start mysql@bootstrap.service
mysql  -e "CREATE DATABASE morpheus CHARACTER SET utf8 COLLATE utf8_general_ci;"
mysql  -e "CREATE USER 'morpheusDbUser'@'%' IDENTIFIED BY '${SSTPAS}';"
mysql  -e "GRANT ALL PRIVILEGES ON *.* TO 'morpheusDbUser'@'%' IDENTIFIED BY '${SSTPAS}';"
mysql  -e "FLUSH PRIVILEGES;"
source /root/.bashrc
sshpass -e scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no /var/lib/mysql/ca.pem /var/lib/mysql/server-cert.pem /var/lib/mysql/server-key.pem  cmpadmin@cmpnode-2:/tmp/
sshpass -e scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no /var/lib/mysql/ca.pem /var/lib/mysql/server-cert.pem /var/lib/mysql/server-key.pem  cmpadmin@cmpnode-3:/tmp/
        echo "####################################################################################"
