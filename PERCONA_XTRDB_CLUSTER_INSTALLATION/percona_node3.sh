...
awk 'BEGIN { ORS = "," } { print }' /tmp/raw.dat > /tmp/cmpnode3.dat
echo "awk command ran sucessfully************"
#       run_cmd sed -i '$d' /tmp/cmpnode3.dat
        run_cmd truncate -s-1 /tmp/cmpnode3.dat
export gcomm=`echo gcomm://$(cat /tmp/cmpnode3.dat)`
...
export wsrepname=`echo wsrep_node_name=$(hostname)`
        ...
        run_cmd rm -f /var/lib/mysql/server-key.pem
                ...
                run_cmd chown mysql:mysql /var/lib/mysql/ca.pem /var/lib/mysql/server-cert.pem /var/lib/mysql/server-key.pem
                run_cmd  sleep 10
